...
def augment_image(image, gt_image):
    if np.random.randint(2) == 0:
        ...
        if np.random.randint(2) == 0:
            # brighten by random factor
            factor = np.random.randint(40)
            image = np.where((255 - image) < factor, 255, image + factor)

        if np.random.randint(2) == 0:
            # darken by random factor
            factor = np.random.randint(-50, 0)
            image = np.where((image + factor) <= 0, 0, image + factor)

        if np.random.randint(2) == 0:
            # random rotation between -15 and 15 degrees
            degrees = np.random.randint(-15, 16)
            image = ndimage.rotate(image, degrees, reshape=False)
            gt_image = ndimage.rotate(gt_image, degrees, reshape=False)
